...
.def tow_counter=r7
.def num_cars = r8				; number of cars on the road
.def row = r9					; keypad
.def col = r10
.def rmask = r11
.def cmask = r12
.def new_speed=r13				; car speed from queue
.def car_speed = r15			; car speed from keypad -10
.def road_status = r16
.def temp1=r17
.def temp2=r18
.def temp3=r19
...
.org INT1addr		; PB
	jmp EXT_INT1
.org INT7addr		; Emergency
	jmp EXT_INT7
.org OVF0addr		; TimerOvF
	jmp Timer0OVF
/* END INTERRUPT VECTOR */

/* CIRCULAR BUFFER WRAP */
; @0: r16+ the register store the pointer
...
/* TO_DECIMAL */
; @0: r0+ containing <=99
; Returns two digits in digit_1, digit_2
.macro to_decimal
	push temp5
	mov temp5,  @0
...
/* SEND CAR TO ROAD */
; Sends car from correct direction if possible
.macro try_send_car
	push zl
	push zh
	push temp3
...
send_car_from_west:
	...
	breq end_try_send_car_jmp_2
	rjmp continue_send

end_try_send_car_jmp_2:
	jmp end_try_send_car

continue_send:
	; pop car from west queue
...
; @0: r0+ containing counter
; @1: maximum value for counter
.macro increment_counter_to
	push temp5
	mov temp5, @0
	cpi temp5, @1
	brlo increment
...
; return value: the first car speed, it will be stored in @1
...
continue:
	adiw z, 1							; go to the car speed we want to iterate
...
car_crash:
	...
	sbrs road_status, EMERGENCY_STATE
...
.macro check_car_and_tow
	...
	; Ignore if no cars
	cpi temp4, 2
	brlo end_check_car_and_tow_jmp
	rjmp continue_check_car

end_check_car_and_tow_jmp:
	jmp end_check_car_and_tow
continue_check_car:
	add zl, temp5								; go to the first car speed in the queue
	ld temp4, z									; load this value to temp4

	tst temp4									; check if car speed is 0
	brne end_check_car_and_tow_jmp					; if not, go to end_check_car_and_tow

	; Once confirm the cars in front have drove off, wait for three seconds
	ldi temp3, 3
	cp tow_counter, temp3								; if three_seconds_count is < 3
	brlo increase_tow_delay_counter_jmp				; increase tow_counter and end
	rjmp continue_tow

increase_tow_delay_counter_jmp:
	jmp increase_tow_delay_counter
continue_tow:

	clr tow_counter										; otherwise, clear tow_counter and tow cars away
...
.macro make_traffic_lights_red
    ...
    timer:
        ...
        cpi temp4, 180                ; has it been 3 minutes?
        brsh set_to_red
        rjmp end_make_traffic_lights_red
...
loop:
	...
	*/
	...
	to_decimal three_minutes_count
	do_lcd_data digit_1
	do_lcd_data digit_2
	subi temp9, -3	
	inc temp8
...
/* SHOW CAR POSITION ANIMATION ON ROAD */
show_car_position: 
	push temp1
	push temp2
	push temp3
...
west_car_pattern:
	...
	; print space
	do_lcd_command 0b1000_0011
	ldi temp1, ' '
	do_lcd_data temp1
	do_lcd_command 0b1000_1011
	ldi temp1, '>'
	do_lcd_data temp1
	do_lcd_data temp1
...
/*
	...
	clr tow_counter								; ensure the tow counter is set to 0
...
refresh:													; one second has passed
	; update timers:
	clr timer_overflow_count
	increment_counter_to three_minutes_count, 180
	increment_counter_to three_seconds_count, 3
